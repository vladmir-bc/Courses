...
# Еще одно допущение, которое мы разберем - независимость остатков
# независимость остатков предполагает, что все наблюдения, которые мы использовали
# они не сгруппированы по отдельным категориям

# для того, чтобы оценить нарушение допущения
# о независимости остатков
# нужно построить график, где по оси Х идут номера наблюдений
# а по оси y идут остатки от модели
...
# гомоскедастичность - требование к моделируемым данным
# согласно которому изменчивость остатков должна быть одинаковой
# на разных уровнях НП, т.е. разброс ошибок должен быть одинаков

# посмотрим на разброс остатков с помощью графика:
ggplot(swiss, aes(x = lm1_fitted, y = lm1_resid)) + 
  geom_point(size = 3)  # по X - предсказанные значения, по Y - остатки

ggplot(swiss, aes(x = lm2_fitted, y = lm2_resid)) + 
  geom_point(size = 3)
########################################################################
# последнее - нормальность распределения остатков. Остатки от линейной 
# регрессии должны быть распределены нормально
...
# распределение остатков не очень нормальное

# можем применить тест Шапиро-Уилка
shapiro.test(lm1$residuals)


# рассмотрим графики для второй модели
...
# распределение остатков более нормальное

# можем применить тест Шапиро-Уилка
shapiro.test(lm2$residuals)

fit <- lm(Education ~ Examination, swiss)
par(mfrow=c(2,2)) ; plot(fit)  # графически показывает основные критерии при анализе
